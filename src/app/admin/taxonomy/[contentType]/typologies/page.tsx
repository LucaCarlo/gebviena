...
export default function TypologiesPage() {
  ...
  const [editForm, setEditForm] = useState({ value: "", label: "", sortOrder: 0 });
  ...
  const handleCreate = async () => {
    ...
    await fetch("/api/typologies", {
      ...
      body: JSON.stringify({ contentType, value: newForm.value, label: newForm.label }),
    });
  ...
  const startEdit = (t: Typology) => {
    setEditingId(t.id);
    setEditForm({ value: t.value, label: t.label, sortOrder: t.sortOrder });
  };
                ...
                <tr key={t.id} className="hover:bg-warm-50 transition-colors">
                  {editingId === t.id ? (
                    <>
                      ...
                      <td className="px-6 py-3 text-warm-400 text-xs">—</td>
                      <td className="px-6 py-3">
                        <input
                          type="number"
                      ...
                      <td className="px-6 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button onClick={() => handleUpdate(t.id)} className="p-1.5 text-green-600 hover:text-green-800"><Check size={16} /></button>
