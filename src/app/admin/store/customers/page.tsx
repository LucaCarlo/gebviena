      ...
      <header className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-warm-500 mt-1.5">
            <strong>{formatNumber(customers.length)}</strong> {customers.length === 1 ? "cliente" : "clienti"}
            {selected.size > 0 && <> · <strong>{formatNumber(selected.size)}</strong> selezionati</>}
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {selected.size > 0 ? (
            <>
              ...
              >
                <Mail size={14} /> Invia email ({formatNumber(selected.size)})
              </button>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    {c.ordersCount > 0 && (
                      <div className="text-[11px] text-warm-500 inline-flex items-center gap-1">
                        <ShoppingCart size={11} /> {formatNumber(c.ordersCount)} {c.ordersCount === 1 ? "ordine" : "ordini"}
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      {c.ordersCount > 0 ? (
                        <span className="inline-flex items-center gap-1 text-warm-700">
                          <ShoppingCart size={12} /> {formatNumber(c.ordersCount)}
                        </span>
                      ) : (
                    ...
                    <td className="px-2 py-3 text-center" onClick={(e) => e.stopPropagation()}>
                      <div className="inline-flex items-center gap-1.5">
                        <button
                          type="button"
                          onClick={() => { setSelected(new Set([c.email])); setBulkEmailOpen(true); }}
                          title={`Invia email a ${c.email}`}
                          className="inline-flex items-center gap-1 px-2.5 py-1 text-xs rounded border border-warm-300 text-warm-700 hover:bg-warm-50 hover:border-warm-900 transition-colors"
                        >
