...
export default function EditProjectPage() {
  ...
  const previewUrl = slug ? `/progetti/${slug}` : null;

  return (
    <div>
      <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
        <div>
          <h1 className="text-2xl font-semibold text-warm-800">Modifica Progetto</h1>
          {isActive !== null && (
      ...
      <div style={{ display: tab === "preview" ? "block" : "none" }}>
        {previewUrl ? (
          <div className="border border-warm-200 rounded-lg overflow-hidden bg-warm-100 flex justify-center"
            ...
            <iframe key={refreshKey} src={`${previewUrl}?_t=${refreshKey}`} title="Anteprima progetto"
