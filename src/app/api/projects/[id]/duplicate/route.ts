...
/**
 * POST — duplica Project. La copia parte come BOZZA (isActive=false),
 * slug = <orig>-copy con suffisso numerico se collide.
 */
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const result = await requirePermission("projects", "edit");
  ...
  for (let i = 2; i < 100; i++) {
    const exists = await prisma.project.findUnique({ where: { slug: newSlug }, select: { id: true } });
    if (!exists) break;
    newSlug = `${baseSlug}-${i}`;
  }
