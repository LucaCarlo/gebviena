...
/**
 * POST — duplica Designer. La copia parte come BOZZA (isActive=false),
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const src = await prisma.designer.findUnique({ where: { id } });
  ...
  for (let i = 2; i < 100; i++) {
    const exists = await prisma.designer.findUnique({ where: { slug: newSlug }, select: { id: true } });
    if (!exists) break;
    newSlug = `${baseSlug}-${i}`;
  }
