 ...
 *
 * Body opzionale: { languages?: string[] }
 *   - non passato / array vuoto / "all" → duplica TUTTE le traduzioni esistenti
 *   - es. ["it"] → solo traduzione italiana
 *   - es. ["it","en"] → IT + EN
 */
export async function POST(
  ...
  // Leggi lingue selezionate dal body (POST può non avere body)
  let requestedLangs: string[] | null = null;
  try {
    const body = await req.json().catch(() => null);
    if (body && Array.isArray(body.languages)) {
      const norm = body.languages.map((s: unknown) => String(s || "").toLowerCase().trim()).filter((s: string) => s);
      if (norm.length > 0 && !norm.includes("all")) requestedLangs = norm;
    }
  } catch { /* nessun body */ }
  ...
  // Filtra le traduzioni in base alle lingue richieste
  const selectedTranslations = requestedLangs
    ? src.translations.filter((tr) => requestedLangs!.includes(tr.languageCode.toLowerCase()))
    : src.translations;

  for (const tr of selectedTranslations) {
