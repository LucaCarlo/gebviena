...
function NewsContent() {
  ...
  const urlArticleType = searchParams.get("_article_type") || "";
  const currentCategory = urlArticleType
    ? (slugsReady ? (slugToValue("news", urlArticleType, lang) || "TUTTI") : "")
    : "TUTTI";
  ...
  const buildNewsUrl = (catValue: string, page: number) => {
    const params = new URLSearchParams();
    if (catValue && catValue !== "TUTTI") {
      const slug = valueToSlug("news", catValue, lang);
      if (slug) params.set("_article_type", slug);
    }
  ...
  const setCategory = (cat: string) => {
    router.push(buildNewsUrl(cat, 1), { scroll: false });
    setTimeout(() => document.querySelector("section.py-8")?.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  };

  const setPage = (page: number) => {
    router.push(buildNewsUrl(currentCategory, page), { scroll: false });
    setTimeout(() => document.querySelector("section.py-8")?.scrollIntoView({ behavior: "smooth", block: "start" }), 100);
  };
