...
export default function AccessoForm({ initialMode = "login" }: { initialMode?: Mode }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Per il LOGIN: usa la lingua del profilo (data.language) — così l'utente
      // atterra automaticamente sull'area pro nella sua lingua predefinita,
      // indipendentemente dalla lingua dell'header al momento del login.
      // Per il REGISTER self-service: usa la lingua corrente del sito.
      const targetLang = (mode === "login" && typeof data.language === "string" && data.language)
        ? data.language
        : lang;
      const prefix = targetLang === "it" ? "" : `/${targetLang}`;
        ...
        {mode === "register" && (
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Field
                label={t(isRequestOnly ? "pro.field.company_agency" : "pro.field.companyExtended")}
                required
                value={form.company}
