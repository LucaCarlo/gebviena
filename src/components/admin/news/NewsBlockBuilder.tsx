...
interface Props {
  ...
  sourceValue?: string;
  /** ID del blocco attualmente selezionato per lo stile (right panel). */
  selectedStyleId?: string | null;
  /** Notifica al parent quando l'utente clicca l'icona Sliders su un blocco. */
  onSelectStyle?: (id: string | null) => void;
}

export default function NewsBlockBuilder({ value, onChange, sourceValue, selectedStyleId, onSelectStyle }: Props) {
            ...
            <div className="bg-white rounded-xl shadow-md border-l-4 border-l-warm-800 border-t border-r border-b border-warm-200 overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b-2 border-warm-200 bg-warm-100">
                ...
                <button
                  type="button"
                  onClick={() => {
                    // Toggle selezione: se clicco sul blocco già selezionato lo deseleziono.
                    onSelectStyle?.(selectedStyleId === b.id ? null : b.id);
                  }}
                  className={`p-1.5 rounded hover:bg-white relative ${
                    selectedStyleId === b.id
                      ? "text-blue-600 bg-blue-50 ring-1 ring-blue-300"
                      : b.style ? "text-warm-900" : "text-warm-500 hover:text-warm-800"
                  }`}
                  title="Apri Stile a destra"
                >
                  <Sliders size={14} />
                  {b.style && Object.keys(b.style).length > 0 && (
                    <span className="absolute top-0 right-0 inline-block w-1.5 h-1.5 rounded-full bg-emerald-500" />
                  )}
                </button>
