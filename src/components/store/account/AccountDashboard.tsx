    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="flex flex-wrap items-end justify-between mb-10 gap-4">
        <div>
          ...
          <h1 className="text-3xl font-light text-warm-900">{t("Ciao", "Bonjour")}, {fullName}</h1>
        ...
        >
          <LogOut size={14} /> {t("Esci", "Déconnexion")}
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Tile href="/account/orders" icon={<Package size={20} />} title={t("I miei ordini", "Mes commandes")} desc={t("Storico, stato e tracking delle spedizioni.", "Historique, statut et suivi des livraisons.")} />
        <Tile href="/account/favorites" icon={<Heart size={20} />} title={t("Preferiti", "Favoris")} desc={t("I prodotti che hai salvato.", "Les produits que vous avez enregistrés.")} />
        <Tile href="/account/profile" icon={<UserCircle2 size={20} />} title={t("Profilo", "Profil")} desc={t("Dati personali, contatti e password.", "Données personnelles, contacts et mot de passe.")} />
      </div>

      <div className="mt-10 border-t border-warm-200 pt-6 text-sm text-warm-500 flex items-center gap-2">
        <Truck size={14} /> {t("Le spedizioni e il tracking sono gestiti dopo la conferma dell'ordine.", "Les livraisons et le suivi sont gérés après la confirmation de la commande.")}
      </div>
    </div>
  );
}
