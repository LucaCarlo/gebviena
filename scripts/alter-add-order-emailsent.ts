/** Aggiunge Order.confirmationEmailSentAt se mancante. Idempotente. */
...
async function main() {
  const table = "Order";
  const col = "confirmationEmailSentAt";
  if (await columnExists(table, col)) { console.log(`✓ ${table}.${col} esiste già`); return; }
  await prisma.$executeRawUnsafe(
    `ALTER TABLE \`${table}\` ADD COLUMN \`${col}\` DATETIME(3) NULL AFTER \`paidAt\``,
  );
