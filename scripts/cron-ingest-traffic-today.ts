...
async function main() {
  ...
  const blockedSet = await loadBlocklist();
  ...
  console.log(`[cron-traffic-today] da inserire=${rows.length} skipBlocked=${skippedBlocked} skipBotPath=${skippedBotPath} cutoff=${new Date(cutoffTs).toISOString()} maxTs=${new Date(maxTs).toISOString()}`);
