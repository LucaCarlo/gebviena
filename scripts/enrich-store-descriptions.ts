 ...
 *
 * Uso: npx tsx scripts/enrich-store-descriptions.ts
 *
 * Idempotente. Sostituisce la marketingDescription esistente con la versione
 * lunga del catalogo qui sotto. Il short rimane riassuntivo (primo paragrafo).
 *
 * Per applicare solo a un raggruppamento specifico:
 *   npx tsx scripts/enrich-store-descriptions.ts "CAFESTUHL"
 */
...
interface ProductCopy {
  ...
  long: string;  // testo lungo per la sezione Descrizione (può contenere markdown leggero: paragrafi separati da blank line, **bold**)
}
...
async function main() {
  ...
  for (const p of targets) {
    // Trova lo StoreProduct cercando la translation per nome esatto
    const tr = await prisma.storeProductTranslation.findFirst({
      where: { name: p.group, languageCode: "it" },
    });
    if (!tr) {
      console.warn(`✗ ${p.group}: translation IT non trovata — skip`);
      notFound++;
      continue;
    }
    ...
    console.log(`✓ ${p.group.padEnd(36)} → marketing ${p.long.length}c, short ${p.short.length}c`);
    updated++;
  }
  console.log("");
  console.log(`Risultato: ${updated} aggiornati, ${notFound} non trovati su ${targets.length} totali.`);
}
